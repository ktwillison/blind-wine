...
var renderer = new THREE.WebGLRenderer({canvas: canvas, antialias: true, alpha: true}); // use webgl renderer (GPU!)
...
Particle.prototype.create_material = function(){
    ...
    this.color.setHSL( Math.random(), .8, .55 );
...
// Add and render Particles (held in data object)
var n = 100;
...
// Define looped draw() function to update location & meshes for each Particle
function draw() {
    for (var i = 0; i<n; i++) {
        data[i].update();
        data[i].update_mesh();
