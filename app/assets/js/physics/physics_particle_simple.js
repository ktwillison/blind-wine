var Particle_Simple = function(width_in, height_in) {
	...
	// Define update function: lazy Euler integration
	this.update = function () {
	    // y direction
	    if ((Math.abs(this.y_v) > 0)) this.y += this.y_v;
	    
	    // x direction
	    if ((Math.abs(this.x_v) > 0)) this.x += this.x_v;
	    
	    // z direction
	    if ((Math.abs(this.z_v) > 0)) this.z += this.z_v;	    
	}
}